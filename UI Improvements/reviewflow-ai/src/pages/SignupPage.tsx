...
const SignupPage = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In a real flow, we might redirect to a 'Verify Email' page, here we just stay or go back to login
    } catch (error: any) {
      toast({ 
        title: "Signup Error", 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-4 rounded-2xl shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 bg-white">

      {/* ─── LEFT PANE (Trial Value Proposition) ─── */}
      <div className="hidden lg:flex w-full bg-[#0F1724] text-white relative flex-col justify-between p-8 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-lg mt-12 mb-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand/20 text-[10px] font-black uppercase tracking-widest text-brand-light border border-brand/30 mb-8 animate-fade-in">
            <Zap className="h-3 w-3 fill-brand-light" /> 14-Day Full Access Trial
          </div>
          
          <h1 className="text-5xl xl:text-6xl font-black leading-[1.05] tracking-tight mb-8 drop-shadow-sm">
          ...
          <div className="space-y-6">
            {[
              ...
              { icon: Globe, text: "Multi-Platform Visibility Boost", color: "text-brand-light" }
            ].map((item, i) => (
        ...
        <div className="relative z-10 mt-8 pt-6 border-t border-white/5 flex flex-col gap-4">
           <div className="flex items-center gap-4">
              <div className="flex -space-x-3">
                {[22, 33, 44, 55].map((id) => (
                  <img key={id} src={`https://i.pravatar.cc/100?img=${id}`} className="h-10 w-10 rounded-full border-2 border-[#0F1724] object-cover" alt="user" />
                ))}
              </div>
              <div className="flex flex-col">
      ...
      <div className="flex-1 flex flex-col justify-center bg-white relative animate-fade-in p-6">
        <div className="absolute top-4 right-4">
          <Link to="/" className="text-muted-foreground hover:text-foreground font-bold">Close</Link>
        </div>
        ...
        <div className="w-full max-w-md mx-auto px-4 sm:px-8 py-6">
          ...
          <div className="space-y-6">
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink-0 mx-4 text-xs font-black uppercase text-muted-foreground/40 tracking-widest">
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <User className="absolute left-3.5 top-[13px] h-4.5 w-4.5 text-muted-foreground/40 group-focus-within:text-brand transition-colors" />
                  <Input
                    id="name"
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Mail className="absolute left-3.5 top-[13px] h-4.5 w-4.5 text-muted-foreground/40 group-focus-within:text-brand transition-colors" />
                  <Input
                    id="email"
                    type="email"
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Lock className="absolute left-3.5 top-[13px] h-4.5 w-4.5 text-muted-foreground/40 group-focus-within:text-brand transition-colors" />
                  <Input
                    id="password"
                  ...
                  <button
                    ...
                    className="absolute right-3.5 top-[13px] h-4.5 w-4.5 text-muted-foreground/40 hover:text-foreground transition-colors"
                  >
            ...
            <div className="text-center mt-10">
              <p className="text-[14px] font-bold text-muted-foreground">
                ...
                <Link to="/login" className="text-brand font-black hover:underline ml-1">
                  Sign In
                </Link>
              </p>
            </div>
          ...
          {/* Bottom links */}
          <div className="mt-6 text-center text-xs font-bold text-muted-foreground/60">
